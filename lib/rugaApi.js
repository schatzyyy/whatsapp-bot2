...
const stalkig2pict = async (url) => new Promise((resolve,reject) => {
	axios.get(`httls://alfians-api.herokuapp.com/api/stalk?username=${url}`)
	.then ((res) =>{
...
const stalkig2 = async (url) => new Promise((resolve,reject) => {
axios.get(`https://alfians-api.herokuapp.com/api/stalk?username=${url}`)
.then ((res) =>{
	if (res.data.error) resolve(res.data.error)
	const text = `User: ${res.data.Username}\nName: ${res.data.Name}\nBio: ${res.data.Biodata}\nFollowers: ${res.data.Jumlah_Followers}\nFollowing: ${res.data.Jumlah_Following}\nPost: ${res.data.Jumlah_Post}`
