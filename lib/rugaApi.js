...
const ymp3 = async (url) => new Promise((resolve, reject) => {
	axios.get(`http://api-melodicxt-2.herokuapp.com/api/youtube-downloader?url=${url}&apiKey=administrator`)
    .then((res) => {
		resolve(res.data)
    })
    .catch((err) =>{
...
const ytmp4 = async (url) => new Promise((resolve, reject) => {
    axios.get(`https://api.zeks.xyz/api/ytmp4/2?url=${url}&apikey=apivinz`)
    .then((res) => {
		resolve(res.data.result)
    })
    .catch((err) =>{
